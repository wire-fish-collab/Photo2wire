...
2. **しきい値二値化による内部線の抽出**
   - 写真をしきい値（スライダー調整・白黒2色プレビュー付き）で2階調化し、
     白黒の境界を線として拾う（Photoshopの「2階調化」フィルター相当）
   - 写真の種類: 「動物・風景（標準）」は通常の輝度で二値化。
     「似顔絵（人の顔）」は肌を明るく持ち上げ（ガンマ補正）、赤み（唇）を
     暗く残すトーンカーブを通してから二値化
   - マスク境界の2px帯のみ除外し、髪の毛など細い部分は保持
   - 旧方式（ポスタリゼーション・DoG・奥行き推定）は `v1/` に保存
     （公開URLの `/v1/` で利用可能）
...
- **複数形式でのダウンロード**:
  ...
  - アニメーションSVG: ブラウザで開くと一筆書きの順に線が描かれていく（ループ再生）
- **描き順アニメーション**: 変換結果の横の「描き順を再生」でその場で確認
